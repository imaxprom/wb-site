#!/bin/bash
# Reconcile FBS supply history, final statuses and sale/return events.
...
LOG="$DATA_DIR/fbs-archive-sync.log"
LOCK_DIR="$DATA_DIR/fbs-archive-sync.lock"
...
if [ "${1:-}" = "--check" ]; then
  ...
  echo "OK: schedule=every-10-minutes"
  exit 0
fi

if ! mkdir "$LOCK_DIR" 2>/dev/null; then
  if [ -d "$LOCK_DIR" ] && find "$LOCK_DIR" -prune -mmin +15 2>/dev/null | grep -q .; then
...
RESP=$(curl --max-time 290 -sS -X POST "$BASE_URL/api/fbs/archive" \
