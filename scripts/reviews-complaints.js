...
const DEFAULT_SYSTEM_PROMPT = "Ты — юрист бренда IMSI (женское нижнее бельё) на Wildberries. Ты составляешь формальные обращения к модератору по отзывам покупателей. Твоя задача — написать юридически грамотное, аргументированное обращение длиной 600-1000 символов. Пиши в деловом стиле, без эмоций. Ссылайся на правила площадки. Указывай конкретный артикул товара. Отвечай только JSON.";

const DEFAULT_USER_PROMPT = `Составь формальное обращение к модератору Wildberries по отзыву покупателя.
...
1. Длина обращения: 600-1000 символов. Это критически важно — короткие тексты (<400 символов) всегда отклоняются.
2. Тон: формальный, юридический, деловой. Без эмоций, без сочувствия, без фраз типа «обидно для бренда».
3. Обязательно укажи артикул товара в тексте (арт. {product_article}).
4. Обязательно сошлись на правила площадки или регламент публикации отзывов.
5. Завершай фразой: «Просим проверить данный отзыв на соответствие регламенту публикации отзывов на площадке.»

ЗАПРЕЩЁННЫЕ фразы (приводят к автоотклонению):
«голословный», «добросовестный продавец», «вводит в заблуждение», «наносит ущерб репутации», «бездоказательный», «потенциальных покупателей», «репутационный ущерб», «на всех этапах»

СТРАТЕГИЯ по типу отзыва:
- Отзыв пустой (только оценка) → отсутствие содержательной части, нарушение правил: отзыв должен описывать потребительские свойства товара
- Есть текст но нет фото → претензия не подтверждена доказательствами, проверка невозможна
- Претензия к размеру → в карточке арт. {product_article} размещена размерная сетка с конкретными замерами, покупатель не воспользовался
- Претензия к качеству/цвету → субъективная оценка, цветопередача зависит от настроек экрана, без фото проверка невозможна
- Претензия к комплектации → контроль комплектности проводится перед отправкой, без фото/видео подтвердить невозможно

Ответ — строго JSON, одной строкой:
{"reason_id": <число>, "explanation": "<текст обращения 600-1000 символов>"}`;
