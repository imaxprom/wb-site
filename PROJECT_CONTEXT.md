...
- Printer troubleshooting belongs in `/printer`; print jobs remain durable across page reloads and computer restarts. For a jam, the Windows helper removes only `MpHub-*` spool documents and reports `queue_ready`. Single labels and QR then use exact operator confirmation; a single WB label claimed as printed must match its scanned barcode. KIZ recovery remains isolated in `/fbs/kiz-archive` by physical position. Physical jams still require a person at the printer.
- For an ordinary multi-label WB batch, `queue_ready` now atomically archives the interrupted attempt as cancelled, creates a fresh job containing every cached label from position 1 and resets initial assembly timestamps until the new full batch is confirmed. Single labels, QR and KIZ keep their precise separate recovery. Completed assembly groups are ordered by their real completion time within category, newest at the bottom, including after reload.
