...
- `fbs.imaxprom.site` exposes only warehouse functions with its own login and organization switcher. Primary modules: `/fbs`, `/fbs/kiz-archive`, `/fbs-stock`, `/printer`; admin can also access employees/settings. `/fbs/kiz-archive` stores applied KIZ values encrypted and isolated by organization, maps GTIN to the legal entity's WB article/size, exposes no raw code or print action, and distinguishes local format validation from optional TrueAPI confirmation.
