...
- Обычная paused/uncertain печать после замятия восстанавливается только через `/printer`: Windows helper удаляет только spool-документы `MpHub-*`; если Windows удерживает документ в `Retained`, helper только для этого явного действия запрашивает права администратора, отключает `KeepPrintedJobs`, перезапускает Print Spooler и при необходимости удаляет только SPL/SHD-файлы с точными ID зависших `MpHub-*` jobs. Статус Windows `Deleting` сам по себе является штатным завершением и не должен считаться отменой. Агент и меню получают красный `queue_paused/Замятие печати`, а точная ошибка восстановления сохраняется как `recovery_error`, пока не устранена. Сотрудник явно выбирает «не вышла — повторить» или «вышла — продолжить». Для обычной WB-этикетки вариант «вышла» обязательно подтверждается точным сканированием её ШК; QR подтверждается оператором. Не возвращать автоматический resume или зелёный статус при paused job.
