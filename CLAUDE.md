...
- Поля сканера в FBS-сборке, контроле маркировки и Архиве КИЗ показывают US-layout при включённой русской раскладке Windows. Для букв регистр берётся из фактического `KeyboardEvent.key` сканера (иначе Caps Lock и scanner case-conversion искажают смешанный регистр WB-этикеток), а физический HID `code` используется для знаков пунктуации. Сервер дополнительно исправляет уже поступившие кириллические символы; ASCII 29/FNC1 Data Matrix нельзя удалять или заменять.
...
- Paused/uncertain печать после замятия восстанавливается только через `/printer`: Windows helper удаляет только spool-документы `MpHub-*`; если Windows удерживает документ в `Retained`, helper только для этого явного действия запрашивает права администратора, отключает `KeepPrintedJobs`, перезапускает Print Spooler и при необходимости удаляет только SPL/SHD-файлы с точными ID зависших `MpHub-*` jobs. Статус Windows `Deleting` сам по себе является штатным завершением и не должен считаться отменой. Агент и меню получают красный `queue_paused/Замятие печати`, а точная ошибка восстановления сохраняется как `recovery_error`, пока не устранена. Для single/QR сотрудник явно подтверждает физический результат; обычная single WB-этикетка сверяется точным сканированием её ШК. КИЗ восстанавливается отдельно по позиции. Не показывать зелёный статус до подтверждённого `queue_ready`.
- Для обычной групповой печати WB действует отдельное согласованное правило: при подтверждённой очистке Windows вся прерванная пачка автоматически создаётся заново из сохранённых файлов и печатается с позиции 1; предыдущая попытка становится `cancelled` и остаётся в аудите, а её физические этикетки выбрасываются. Это не применяется к `single`, QR и КИЗ. На сборке готовые группы сортируются по фактическому времени завершения внутри своей категории: последняя завершённая всегда самая нижняя.
- `/fbs/archive` — organization-scoped архив FBS-поставок. Не создавать отдельную копию рабочих заказов: источник состава — `fbs_fulfillment_orders`/`fbs_fulfillment_supplies`, старые задания дополняются WB archive API, а состав каждой поставки проверяется через `order-ids`. `declined_by_client` означает раннюю отмену, `canceled_by_client` — отказ при получении; возврат после выкупа подтверждается отдельно по `srid=rid` из `fbs_sales_events`/финансовых отчётов. История domestic FBS фиксирует момент, когда MpHub увидел новый статус, и не должна выдаваться за точное время перехода WB в прошлом.
- Доступность `/fbs/kiz-archive` управляется отдельно для каждого юрлица ключом `settings.fbs_kiz_archive_enabled`. Выключение скрывает пункт в FBS-портале, блокирует страницу и API, но не удаляет сохранённые КИЗ. На production для `public` включено, для `organization_2` выключено.
...
- Удалённое администрирование складского Windows-компьютера настроено и проверено после перезагрузки: с технической VM `codex-cli` используется Tailscale/OpenSSH alias `warehouse-pack-01`, отдельная key-only административная учётная запись `mphub-support`. Не выводить ключи или конфигурацию доступа. Физическое замятие этикетки всё равно требует сотрудника рядом.
...
- SMS-авторизация WB через `scripts/wb-seller-login.py` синхронизирует `authorizev3`, `wbx-validation-key`, `wb_seller_lk` в `review_accounts` по `supplier_id`. После отправки SMS UI не держит длинный HTTP-запрос: `/api/wb/auth/progress` коротко опрашивает фоновую сессию до выбора кабинета или сохранения токенов. Если у номера несколько юрлиц, выбор должен запрашиваться у пользователя, не выбирается автоматически. Список кабинетов распознаётся по стабильным полям WB `ID`/`INN`, а не только по русским префиксам `ИП`/`ООО`: иностранные юридические лица также должны отображаться и выбираться. В токенах `supplierOwnerId=Z-Soid` — ID кабинета из UI и `oid` API-ключа, а `supplierId=Z-Sfid` — внутренний ID поставщика; проверка соответствия API/LK сравнивает `oid` только с `Z-Soid`, с fallback на старый `supplierId` для legacy-токенов.
...
- С 2026-08-17 monitor полностью organization-scoped: cron пишет `data/organizations/<id>/data-health-cron.json` и `monitor-status.json`, а `/api/monitor/summary|status|logs|data-health` читает только выбранное юрлицо. Общие `public/data/monitor/status.json`/`auth-status.json` не являются источником tenant-статуса; `status.json` остаётся только совместимым зеркалом org 1. Проверки неприменимых модулей имеют `disabled`, ошибки считаются по timestamp только за реальные 24 часа, ответы log API маскируют credentials.
- Runtime sync не должен выполнять owner-DDL (`CREATE/ALTER/INDEX`) в tenant schema. Все таблицы создаются миграциями; рабочий код делает `to_regclass`-проверку и выдаёт явную `Database migration missing`, сохраняя запрет `CREATE` для роли `mphub_app`.
