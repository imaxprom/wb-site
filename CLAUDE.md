...
- `/fbs/kiz-archive` — архив уже нанесённых КИЗ. Полный Data Matrix хранится только зашифрованно в organization-scoped `fbs_kiz_archive`, не возвращается в UI и не имеет функции печати. Локальная проверка формата/GTIN отображается отдельно от подтверждения TrueAPI; без `FBS_KIZ_TRUEAPI_URL` + `FBS_KIZ_TRUEAPI_TOKEN` нельзя показывать зелёный онлайн-статус.
