...
function assemblySgtinErrorMessage(order: Order, status?: AssemblyMarkingStatus) {
  ...
  const detail = (order.metadata_decisions || []).find((item) => {
    ...
    return type === "sgtin" && Boolean(decision) && !["filled", "pending", "deadlineexceeded", "required", "optional", "missing", "empty"].includes(decision);
  });
...
export default function FbsPage() {
  ...
  const metaPending = (order: Order) => (order.metadata_decisions || []).some((detail) =>
    (detail.key === "sgtin" || detail.type === "sgtin" || detail.name === "sgtin")
    && ["pending", "deadlineexceeded"].includes((detail.decision || detail.status || "").toLowerCase().replace(/[^a-z0-9]/g, "")),
  );
    ...
    setAssemblyMarkingStatus((current) => {
      ...
      for (const order of resolved) {
        next[order.order_id] = sgtinReady(order)
          ...
          : { state: "error", message: "WB отклонил код — пересканируйте", updatedAt: new Date().toISOString() };
      }
      return next;
    });
    const rejected = resolved.find(metaRejected);
    if (rejected) {
      setAssemblyMarkingMessage(`${fbsLabelText(rejected)}: WB отклонил «Честный знак» — пересканируйте код`);
      playScanTone(false);
    } else {
  ...
  return (
    <main className="fbs-portal-content space-y-5">
      ...
      {activeStep !== "tasks" && <section className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-4">
        <div className={`grid w-full items-center gap-2 ${activeStep === "assembly" && activeSupply && assemblySgtinOrders.length > 0 ? "lg:grid-cols-[auto_minmax(260px,1fr)_145px_145px_145px]" : "max-w-[760px] grid-cols-[auto_minmax(0,1fr)]"}`}>
          ...
          {activeStep === "assembly" && activeSupply && assemblySgtinOrders.length > 0 && <>
            <button type="button" onClick={() => setAssemblyStatusPanel("accepted")} className="col-span-2 flex h-[48px] min-w-0 items-center rounded-lg bg-emerald-500/10 px-3 text-left transition hover:bg-emerald-500/20 sm:col-span-1 lg:col-auto"><span className="w-10 shrink-0 text-xl font-bold tabular-nums text-emerald-500">{assemblySgtinAccepted}</span><span className="whitespace-nowrap text-sm font-medium text-[var(--text)]">Принято WB</span></button>
            <button type="button" onClick={() => setAssemblyStatusPanel("pending")} className="col-span-2 flex h-[48px] min-w-0 items-center rounded-lg bg-amber-500/10 px-3 text-left transition hover:bg-amber-500/20 sm:col-span-1 lg:col-auto"><span className="w-10 shrink-0 text-xl font-bold tabular-nums text-amber-500">{assemblySgtinPending}</span><span className="whitespace-nowrap text-sm font-medium text-[var(--text)]">Проверяется</span></button>
            <button type="button" onClick={() => setAssemblyStatusPanel("errors")} className="col-span-2 flex h-[48px] min-w-0 items-center rounded-lg bg-red-500/10 px-3 text-left transition hover:bg-red-500/20 sm:col-span-1 lg:col-auto"><span className="w-10 shrink-0 text-xl font-bold tabular-nums text-red-500">{assemblySgtinErrors}</span><span className="whitespace-nowrap text-sm font-medium text-[var(--text)]">Ошибки</span></button>
          </>}
        </div>
