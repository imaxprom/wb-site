...
function calculateLocalizationMetrics() {
  ...
  const maxDateRow = db.prepare("SELECT date(MAX(date)) AS max_date FROM shipment_orders").get() as { max_date: string | null };
  ...
  const rows = db.prepare(`
    ...
    WHERE date(date) >= date(?)
      AND date(date) <= date(?)
      AND TRIM(COALESCE(article_wb, '')) != ''
  `).all(window.startDate, window.endDate) as LocalizationOrderRow[];
