...
import { getDb, getLastWeekCorrection, getUserOverrides, getUserSettings, initShipmentTables } from "@/lib/shipment-db";
...
): Pick<StockRow, "packing_days" | "packing_multiplier" | "base_orders_qty" | "base_sales_qty" | "buyout_rate" | "trend_multiplier" | "trend_direction" | "target_sales_qty" | "target_sales_45d" | "wb_stock_qty" | "warehouse_required_units" | "plan_pack_units" | "plan_pack_boxes"> {
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const settings = userId ? getUserSettings(userId) : {};
    ...
    if (!tableExists("warehouse_ready_stock")) {
    ...
    const productSizesByArticle = buildProductSizesByArticle(db);
    const wbStockByBarcode = buildWbStockByBarcode(db);
    const ordersByBarcode = buildOrdersByBarcode(db, packingDays);
    const includedArticles = new Set(rawRows.map((row) => String(row.article_wb)));
    const trendStats = buildArticleTrendByArticle(db, productSizesByArticle, loadedDays, disabledByArticle, includedArticles);
    const trendByArticle = trendStats.byArticle;
    const autoBuyoutByArticle = effectiveSettings.buyoutMode === "auto"
      ? buildAutoBuyoutRateByArticle(db)
    ...
    const lastRun = tableExists("warehouse_sync_runs")
      ? db.prepare(`
          SELECT
            id,
            spreadsheet_title,
