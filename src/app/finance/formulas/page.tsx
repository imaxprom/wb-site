...
const FORMULAS = [
  ...
  {
    title: "Налог (УСН)",
    formula: "(Сумма реализации − НДС) × ставка УСН",
    source: "Настройки налогов",
    note: "Упрощённый налог считается от дохода за вычетом НДС — так вы не платите налог на налог. Ставка зависит от региона и режима: «Доходы» — от 1% до 6%, «Доходы минус расходы» — от 5% до 15%. Укажите вашу ставку в настройках налогов",
  },
  {
    title: "НДС к уплате",
    formula: "Сумма реализации × ставка НДС ÷ (100 + ставка НДС)",
    source: "Настройки налогов",
    note: "НДС уже включён в розничную цену, поэтому он извлекается делением, а не начисляется сверху. Пример: товар за 1050₽ при ставке 5% → НДС = 1050 × 5 ÷ 105 = 50₽. С 2025 года для УСН с оборотом выше 60 млн ₽ действуют ставки 5% или 7%",
  },
  {
    title: "Себестоимость",
...
const NUANCES = [
  ...
  { q: "Почему НДС считается делением, а не умножением?", a: "НДС уже включён в розничную цену товара. Чтобы извлечь его, нужно делить: 1050₽ × 5 ÷ 105 = 50₽. Если умножить 1050 × 5% = 52.50₽ — это переплата на 2.50₽ с каждой единицы." },
  { q: "Почему данные отстают на 1–3 дня?", a: "WB формирует отчёты понедельно с задержкой. Последние дни могут быть неполными — это нормально." },
  { q: "Что такое «Сумма к выплате продавцу»?", a: "Это сумма, которую WB переводит вам на расчётный счёт после удержания комиссии, СПП и эквайринга. Важно: налоги (УСН и НДС) считаются от полной суммы реализации, а не от суммы к выплате." },
  ...
  return (
    <div className="space-y-5">
      ...
      <p className="text-center text-[10px] text-[var(--text-muted)] py-2 border-t border-[var(--border)]">
        Формулы обновлены 07.04.2026 · Сверены с кодом расчётов MpHub
      </p>
    </div>
  );
}
