...
function getWeeklyDb(): Database.Database | null {
...
function getJamTotal(d: Database.Database, dateFrom: string, dateTo: string): number {
  const row = d.prepare(jamDedupSql()).get(dateFrom, dateTo) as Record<string, number>;
  return row.total || 0;
}
...
function getPnlFromExcel(wdb: Database.Database, dateFrom: string, dateTo: string, nmId?: number): {
  ...
  const cogsHistory = getCogsHistoryMap();
  let cogs = 0;
  ...
  for (const row of qtyRows) {
    cogs += row.qty * getCostForDate(cogsHistory, row.barcode, row.sale_dt);
  }
  let cogsReturns = 0;
  ...
  for (const row of retQtyRows) {
    cogsReturns += row.qty * getCostForDate(cogsHistory, row.barcode, row.sale_dt);
  }
...
export function getPnl(dateFrom: string, dateTo: string, nmId?: number): PnlResult {
  ...
  if (wdb && !nmId) {
    ...
    if (excelData) {
      ...
      const jam = getJamTotal(d, dateFrom, dateTo);
      ...
      return {
        ...
        commission, logistics: svcRow.logistics, storage: svcRow.storage,
        penalty: svcRow.penalty, acceptance: svcRow.acceptance,
        other_services: other, jam, rebill: svcRow.rebill || 0,
        total_services: commission + svcRow.logistics + adRow.total + other + jam,
        cogs, ad_spend: adRow.total, orders_sum: ordRow.total,
        sales_qty: salesRow.qty, returns_qty: returnsRow.qty,
  ...
  if (wkDb) {
    try {
      const loyaltyRange = wkDb.prepare(`
        SELECT COALESCE(SUM(loyalty_compensation), 0) as total FROM weekly_rows
        WHERE period_from >= ? AND period_to <= ?
  ...
  return {
    realization: salesRow.rpwd - returnsRow.rpwd,
    sales_rpwd: salesRow.rpwd, returns_rpwd: returnsRow.rpwd,
    retail_amount: salesRow.ra - returnsRow.ra,
    loyalty_compensation: loyaltyComp,
    ppvz: salesRow.ppvz - returnsRow.ppvz, commission,
    logistics: svcRow.logistics, storage: svcRow.storage,
    penalty: svcRow.penalty, acceptance: svcRow.acceptance,
    other_services: other, jam, rebill: svcRow.rebill || 0,
    total_services: commission + svcRow.logistics + adRow.total + other + jam,
    cogs: salesRow.cogs - returnsRow.cogs,
    ad_spend: adRow.total, orders_sum: ordRow.total,
    sales_qty: salesRow.qty, returns_qty: returnsRow.qty,
    net_qty: salesRow.qty - returnsRow.qty,
  };
}
...
export function getDaily(dateFrom: string, dateTo: string, nmId?: number): DailyDbRow[] {
  ...
  const adsDaily = d.prepare(`SELECT date, SUM(amount) as total FROM advertising WHERE date >= ? AND date <= ? GROUP BY date`).all(dateFrom, dateTo) as Record<string, number>[];
  const ordersDaily = d.prepare(`SELECT date, order_sum FROM orders_funnel WHERE date >= ? AND date <= ?`).all(dateFrom, dateTo) as Record<string, number>[];
  ...
  for (const dt of Array.from(allDates).sort()) {
    ...
    result.push({
      date: dt as unknown as string, orders_rub: ord.order_sum,
      sales_rub: s.rpwd, returns_rub: ret.rpwd, realization,
      sales_qty: s.qty, returns_qty: ret.qty, net_qty: s.qty - ret.qty,
      commission: comm.comm, logistics: svc.logistics, storage: svc.storage,
      penalty: svc.penalty, ad_spend: ad.total, cogs: s.cogs_sum, profit,
    });
  }
  return result;
}
...
export async function getDailyPg(dateFrom: string, dateTo: string, nmId?: number): Promise<DailyDbRow[]> {
  ...
  const ordersDaily = await pgRows<Record<string, number | string>>(
    `SELECT date, order_sum FROM orders_funnel WHERE date >= ? AND date <= ?`,
    [dateFrom, dateTo]
  );
...
export function getFilters() {
  if (filtersCache && Date.now() - filtersCacheTime < FILTERS_CACHE_TTL) return filtersCache;
  const d = getDb();
  ...
  const result = {
    suppliers: [...suppliers].sort(), brands: [...brands].sort(),
    subjects: [...subjects].sort(), sizes: [...sizes].sort(),
